...
// 1. Group by sector
const sectorMap = Investedvalue.reduce((acc, curr) => {
  const { sector, visitors } = curr;
  if (!acc[sector]) {
    acc[sector] = { sector, visitors: 0 };
  }
  acc[sector].visitors += visitors;
  return acc;
}, {} as Record<string, { sector: string; visitors: number }>);

// 2. Total invested value
const totalInvested1 = Object.values(sectorMap).reduce((sum, item) => sum + item.visitors, 0);

// 3. Add percentage and unique colors
const colors = [
  "#FF6384", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40", "#C9CBCF", "#B2FF66", "#4D5360"
];

const groupedBySector = Object.values(sectorMap).map((item, index) => ({
  ...item,
  percentage: ((item.visitors / totalInvested1) * 100).toFixed(1), // 1 decimal place
      ...
      <div className="flex-1 px-4 py-6 overflow-auto flex flex-col gap-6 md:px-6">
        ...
        <div className='flex flex-col gap-4 border border-gray-200 rounded-lg p-4 bg-white shadow-sm'>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={groupedBySector}>
              <XAxis dataKey="sector" />
              <YAxis />
              <Tooltip
                formatter={(value: number, name: string, props: any) =>
                  [`₹${value.toLocaleString("en-IN")}`, "Investment"]
                }
                labelFormatter={(label: string) => `Sector: ${label}`}
              />
              <Legend />
              <Bar dataKey="visitors" isAnimationActive fill="#8884d8">
                {groupedBySector.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.fill} />
                ))}
              </Bar>
            </BarChart>
